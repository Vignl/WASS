...
if (isset($_POST['login'])) {
    ...
    if ($ret > 0) {
        ...
        $userQuery = mysqli_query($con, "SELECT IsEnabled FROM tbluser WHERE ID = '".$ret['ID']."'");
        $userRet = mysqli_fetch_array($userQuery);
        
        if ($userRet['IsEnabled'] == 1) {
            $_SESSION['bpmsuid'] = $ret['ID'];
            header('location:index.php');
        } else {
            $msg = "Your account is disabled. Please contact the administrator.";
        }
    } else {
        $msg = "Invalid Details.";
    }
...
<html lang="en">
  <head>
    ...
    <title>J. Castillon Dental Clinic | Login</title>
...
?>
               ...
               <?php } ?> </div>
                ...
                <h4 style="padding-bottom: 20px;text-align: center;color: #28a745;">Login</h4>
                    <form method="post">
                    <p style="font-size:16px; color:red" align="center"> <?php if($msg){
                        ...
                        <div class="twice-two" style="padding-top: 30px;">
                          <a class="link--gray" style="color: #28a745;" href="forgot-password.php">Forgot Password?</a>
                          <a class="link--gray" style="color: #28a745;" href="signup.php">Dont' have an account?</a>
                        </div>
